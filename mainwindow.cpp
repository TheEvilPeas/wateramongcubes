...
void MainWindow::handleRandomButtonClicked() {
    int rows;
    int cols;
    // Проверка, что поля для ввода строк и столбцов пусты
    if (rowsInput->text().isEmpty() && colsInput->text().isEmpty()) {
        // Инициализация генератора случайных чисел
        srand(time(0));

        // Генерация случайного числа от 0 до 10 для количества строк и столбцов
        rows = rand() % 50+3;
        cols = rand() % 50+3;
...
/**
 * @brief Обработчик нажатия на кнопку "Сохранить".
 * сохраняет матрицу в виде файла bin или txt
 */
...
/**
 * @brief Обработчик нажатия на кнопку "Загрузить".
 * загружает матрицу из файла .bin
 */


void MainWindow::handleLoadButtonClicked() {
    QString fileName = QFileDialog::getOpenFileName(this, "Загрузить файл", "", "BIN файлы (*.bin);;Текстовые файлы (*.txt)");
...
void MainWindow::handleCalculationComplete(ll result, const vector<vector<int>>& workingMatrix) {
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            QString difference = QString::number(newText.toInt() - oldText.toInt());

            if (oldText != newText) {
                QString displayText = newText;
                textItem->setToolTip("было воды:"+oldText+"\nналилось воды:"+difference);
